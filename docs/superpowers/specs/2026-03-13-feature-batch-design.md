...
- Numeric: min, max, avg, median, std dev, histogram (10 buckets)
...
- `src/main/adapters/` — add standalone `getColumnStats()` function per adapter
...
**Adapter changes:** Standalone functions, not added to `DatabaseAdapter` interface.
...
**Modified files:**
- `src/renderer/src/components/data-table.tsx` — apply blur CSS to masked columns
- `src/renderer/src/components/editable-data-table.tsx` — apply blur CSS
- `src/renderer/src/lib/` (export utilities) — intercept masked columns on export
...
**Architecture:**
1. CSV parsing in renderer (papaparse, streaming)
2. Renderer sends parsed rows in batches to main via IPC `db:import-csv`
3. Main process adapter builds INSERT statements (or COPY for Postgres) in batches within transaction
4. Progress reported via IPC push events `db:import-progress`
...
- `src/main/ipc/import-handlers.ts` — IPC handlers for import + progress
...
**Adapter additions:** Standalone `importBatch()` function per adapter. Postgres uses COPY FROM STDIN, MySQL uses batch INSERT, MSSQL uses bulk insert.

**New dependency:** `papaparse` (renderer devDependency)
...
3. **Preview & Execute** — preview 5 rows. Show INSERT SQL. Progress bar. Summary.
...
3. Rows generated in memory, inserted in batches (reuses import batch logic from Feature 3)
4. Progress via IPC push events
...
- Tab store: new tab type `'data-generator'`

**New dependency:** `@faker-js/faker` (main process dependency)
...
**UI:** Dedicated tab (type `notifications`) with three sections:
...
3. Client forwards `notification` events to renderer via IPC push `notifications:event`
4. Auto-reconnect with exponential backoff on disconnect, re-subscribes all channels
5. Event history persisted in `better-sqlite3` local storage, capped at 10,000 events per connection

**IPC channels:**
- `notifications:subscribe(connectionId, channel)` — LISTEN
- `notifications:unsubscribe(connectionId, channel)` — UNLISTEN
- `notifications:send(connectionId, channel, payload)` — NOTIFY
- `notifications:get-channels(connectionId)` — list active
- `notifications:get-history(connectionId, limit)` — fetch persisted
- `notifications:clear-history(connectionId)` — purge
- `notifications:event` — push from main → renderer via `webContents.send`

**PostgreSQL only.** Feature hidden for non-PG connections in sidebar nav.

**New files:**
- `packages/shared/src/index.ts` — add `NotificationEvent`, `NotificationChannel`, `NotificationStats` types
- `src/main/ipc/notification-handlers.ts` — IPC handlers
- `src/main/notification-listener.ts` — dedicated pg.Client manager, reconnect, event forwarding
- `src/renderer/src/stores/notification-store.ts` — events, channels, stats, history
- `src/renderer/src/components/notifications-panel.tsx` — tab UI
- Tab store: new tab type `'notifications'`
...
**Auto-refresh:** Configurable (2s/5s/10s/30s/off) via dropdown in tab header. Each card refreshes independently. Manual refresh per card.

**Architecture:**
1. Each card maps to one IPC call running a diagnostic SQL query
2. Adapter-specific diagnostic functions: `getActiveQueries()`, `getTableSizes()`, `getCacheStats()`, `getLocks()`
3. Renderer polls on interval via `setInterval`, stops when tab inactive
4. Kill query via new IPC `db:kill-query(connectionId, pid)`
...
- `src/main/adapters/` — standalone diagnostic functions per adapter
...
- Tab store: new tab type `'health-monitor'`

---

## Cross-Cutting Concerns

**New tab types to register:**
- `'data-generator'` (Feature 4)
- `'notifications'` (Feature 5)
- `'health-monitor'` (Feature 6)

**Shared patterns:**
- Features 3 & 4 share batch insert logic (build once in Feature 3, reuse in Feature 4)
- Features 5 & 6 both use IPC push events (main → renderer via `webContents.send`)
- All new tab types follow existing tab store pattern

**No breaking changes** to existing interfaces or behavior. All features are additive.
