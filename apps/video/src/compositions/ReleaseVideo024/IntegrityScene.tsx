...
const ChecklistRow: React.FC<{ label: string; detail: string; delay: number }> = ({
  label,
  detail,
  delay,
}) => {
  ...
  const tick = spring({ frame: frame - delay - 8, fps, config: { damping: 10, stiffness: 140 } })
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {ITEMS.map((item, i) => (
          <ChecklistRow key={item.label} label={item.label} detail={item.detail} delay={20 + i * 22} />
        ))}
      </div>
    </AbsoluteFill>
  )
