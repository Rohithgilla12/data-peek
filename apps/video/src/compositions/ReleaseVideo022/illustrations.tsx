...
const TimelineRow: React.FC<{
  ...
  >
    <span style={{ width: 110, fontSize: 11, color: brand.textMuted }}>{label}</span>
    <span style={{ flex: 1 }} />
    <span
      style={{
        fontSize: 11,
...
export const E2EFortressIllustration: React.FC = () => {
  ...
  const cellOpacity = (i: number) => {
    if (i < 4) return interpolate(frame, [5, 15], [0, 1], { extrapolateRight: 'clamp' })
  ...
  const counterValue = Math.min(25, Math.max(4, Math.floor(interpolate(frame, [20, 100], [4, 25]))))
