...
import type {
  ConnectionConfig,
  SchemaInfo,
  StoredChatMessage as SharedStoredChatMessage
...
export function AIChatPanel({
  ...
  React.useEffect(() => {
    ...
    const saveSession = async () => {
      try {
        // Convert AIChatMessage to SharedStoredChatMessage format
        const storedMessages: SharedStoredChatMessage[] = messages.map((m) => ({
          id: m.id,
          role: m.role,
          content: m.content,
          responseData: m.responseData
            ? m.responseData.type === 'query'
              ? { ...m.responseData, explanation: m.responseData.explanation ?? '' }
              : m.responseData
            : null,
