...
| `run_query`         | read-only       | Runs a single SELECT/EXPLAIN, capped at 500 rows    |
...
Now just ask questions. The agent calls `list_connections`, then `run_query` — reads run in a read-only, rolled-back transaction. Credentials are never exposed:
...
```text
// run_query: SELECT plan, count(*) FROM organizations GROUP BY plan
 plan         orgs
 free         2
 enterprise   2
 starter      2
